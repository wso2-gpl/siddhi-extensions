...
//        System.out.println("DEBUG: informationBuffer = " + informationBuffer+" informationBuffer.containsKey(id) = "+informationBuffer.containsKey(id)+" id = "+id);
        if (informationBuffer.containsKey(id) && !informationBuffer.get(id).equals(currentInformation)) {
//            System.out.println("DEBUG: informationBuffer.get(id) : "+informationBuffer.get(id)+" currentInformation = "+currentInformation);
            returnValue = true;
        }
...
/*    protected Object process(Object obj) {
        ...
        if (informationBuffer.containsKey(id)) {
            String previousInformation = informationBuffer.get(id);
            if (!previousInformation.equals(currentInformation)) {
                informationBuffer.put(id, currentInformation);
                returnValue = true;
            }
            else {// If equals no need to put the same information again , so no need to put a else block here ;)
                informationBuffer.put(id, currentInformation);
            }
